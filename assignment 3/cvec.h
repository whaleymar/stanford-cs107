...
typedef struct VectorImplementation vector;

struct VectorImplementation{
	void* elems; // pointer to element array
	size_t nelems; // number of elements stored
	size_t elemsz; // size of elements to be stored
	size_t nalloc; // max # of elements that can be stored w/ current memory allocation
	VectorFreeFunction clean; // function to free all memory associated with this struct
};

/**
 * Function: VectorCreate
 * Usage: vector myFriends;
 * VectorCreate(&myFriends, sizeof(char *), StringFree, 10);
 * -------------------
